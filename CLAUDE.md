...
This repository contains Plutarch implementations for the UPLC-CAPE benchmark, which compares UPLC programs generated by different compilers and optimization strategies. The project submits 7 different implementations demonstrating various optimization strategies including fixed-point combinators and pre-computed sequences.
...
```bash
cabal build all                    # Build all 7 executables
cabal run factorial-size           # Run size-optimized factorial
cabal run factorial-exbudget       # Run budget-optimized factorial
cabal run factorial-naive          # Run naive factorial
cabal run fibonacci-size           # Run size-optimized fibonacci
cabal run fibonacci-prepacked      # Run pre-computed fibonacci
cabal run fibonacci-exbudget       # Run budget-optimized fibonacci
...
4. Write to `.uplc` file in the executable's source directory

All executables use: `compileToUplc :: FilePath -> ClosedTerm a -> IO ()`

Each executable passes a path to its own directory (e.g., `"exe/factorial-size/factorial_size.uplc"`) to ensure UPLC files are generated alongside their source code and tracked in git.
...
Each executable generates its UPLC file in its own source directory (`exe/<executable-name>/*.uplc`). The output filename is specified in the executable's `Main.hs` and follows the pattern of the implementation name (e.g., `factorial_size.uplc`, `fibonacci_naive_recursion.uplc`).

**Pattern**: Each `Main.hs` calls `compileToUplc "exe/<executable-name>/<output-file>.uplc" <term>`

These UPLC files are tracked in git as part of the benchmark submissions.
